...
aphids = read.csv('./data/Rhopalosiphum_padi_data.csv',as.is=T,check.names=F,header=T) #387 rows
...
writeOGR(shp,dsn='./shapefiles/R.padi_sites_phenology.shp',layer='R.padi_sites_phenology.shp',driver='ESRI Shapefile',overwrite=T)
...
write.csv(output,'R.padi_LMM_output.csv',quote=F,row.names=F)
