...
aphids = read.csv('./data/Aphis_glycines_data.csv',as.is=T,check.names=F,header=T) #371 rows
...
shp@data$First.male.flight = apply(array(shp@data$Site),1,function(x){mean(aphids$First.male.flight[which(aphids$Site==x)],na.rm=T)})
shp = shp[which(!is.na(shp@data$Synchrony)),]
writeOGR(shp,dsn='./shapefiles/A.glycines_sites_phenology.shp',layer='STN_sites_phenology.shp',driver='ESRI Shapefile',overwrite=T)
...
fit5 = lmer(First.male.flight ~ tmean.ONDJFM*ppt.ONDJFM + tmean.AMJJAS*ppt.AMJJAS + Prop.cropland + Edge.density + Prop.forestwetland + (1|Year) + (1|Latitude), data=aphids2)
sm5 = summary(fit5)
av5 = car::Anova(fit5)
plot(fit5)
...
responses = c('Synchrony','F10','F50','F90','First.male.flight','First.flight','Last.flight','Last-First','F90-F10')
fits = list(sm1,sm2,sm3,sm4,sm5,sm6,sm7,sm8,sm9)
anovas = list(av1,av2,av3,av4,av5,av6,av7,av8,av9)
...
write.csv(output,'A.glycines_LMM_output.csv',quote=F,row.names=F)
